    ...
    jQuery.ajax({
        ...
        success: function(response) {
            if(response.data.length){
                jQuery('#noItemInVault').remove();
                jQuery.each(response.data, function(index, folder) {
                    jQuery('#vaultItemsShow').append(`<div id='vaultItems' class=" hover:bg-gray-50 ">
                    <div class="flex items-center space-x-5 text-gray-500 pl-5 py-3">
                        ...
                        <a href="#" class="text-sky-600 hover:text-sky-700 hover:underline cursor-pointer">`+folder.name+`</a>
                    </div>
                    <hr>
                </div>`)
                })
            } else {
                jQuery('#noItemInVault').show();
            }
